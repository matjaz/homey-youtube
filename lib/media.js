...
var MAX_PLAYLISTS_ITEMS = 50 // Number of playlists item to fetch
var pollingInterval
  ...
  Homey.manager('media').on('search', function (parsedQuery, callback) {
    ...
    search.searchQuery(parsedQuery.searchQuery, function (err, result) {
      ...
      result.items.forEach(function (item) {
        idList += (idList === '') ? item.id.videoId : (',' + item.id.videoId)
      })
      youtube('videos', {
    ...
    oauth.getAccessToken(function (err, accessToken) {
      ...
      http.json({
        ...
        if (response && response.items) {
          ...
          Promise.all(playlistRequests).then(() => {
...
exports.setTrack = function (item, callback) {
  ...
  youtubemp3.getDownloadUrl(id, function (err, url, length) {
    if (err) {
      callback(err)
      return
    }
    ...
    http.json({
      ...
      response.items.forEach(function (item) {
        if (item.snippet.resourceId.videoId) {
          playlistIdList += (playlistIdList === '') ? item.snippet.resourceId.videoId : (',' + item.snippet.resourceId.videoId)
        }
      })
      ...
      youtube('videos', {
        ...
        if (err) {
          ...
          var playlistItems = result.items.map(function (item) {
            return exports.parseTrack(item)
          })
