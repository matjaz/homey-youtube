    ...
    search.searchQuery(parsedQuery.searchQuery, function (err, result) {
      ...
      youtube('videos', {
        ...
        if (err) {
          ...
          }).filter(function (item) { return item.duration < MAX_VIDEO_DURATION })
          callback(null, lastResults) // Filter results that are longer than 2H because the MP3 API does not support this
        }
      })
    })
  })
